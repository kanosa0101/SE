...
首次启动会构建 Vue 静态资源，并由 FastAPI 同时提供 API 与前端页面。SQLite 文件位于 cvinsight-data Docker volume 中。

## 华为云 CodeArts 操作边界

1. 将仓库导入 CodeArts，默认分支使用 main，构建分支使用 dev。
2. 构建任务使用仓库根目录的 Dockerfile 构建镜像，并将容器端口 8000 映射到云主机端口。
3. 在 CodeArts/云主机环境变量中填写 CVINSIGHT_LOOKUP_URL（可选）和 CVINSIGHT_CORS_ORIGINS，不要把真实密钥写入仓库。
...
- 在线检索源未配置时，界面应提示“未配置在线检索源”，本地论文管理功能仍可用。
