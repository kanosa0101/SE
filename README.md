...
- 已知问题（记录在案，当前未处理）：
  - 数据扩容到 2.6 万条后统计接口首查较慢：已完成两轮优化（元组流聚合 + 领域映射缓存 + keyword_id 索引），topics/trends/evolution 降到 3-4s、graph 约 7s；graph 自连接在更大规模下仍会线性变慢，方向是共现对物化（预计算表）或换用支持图查询的存储。前端已有 15s 超时兜底。
  - ECCV 2020/2022/2024 在 CVF 未收录（404），ECCV 只有 2018 一年样本，比较 ECCV 趋势时需注明。
...
3. 热门方向排名：作业口径是"热门领域/研究方向"而非出现最多的词。`research_area_for`（backend/app/services/keywords.py）把细粒度关键词按词元映射到 15 个研究领域（Object Detection、Diffusion & Generative Models、Vision-Language & Multimodal 等），Top 10 / 趋势 / 年度演变按领域的覆盖论文数（并集去重）排序；未映射到领域的泛化载体词（image、model、deep 等）不参与排名。关键词本体与共现图谱保持原样，不受映射影响。
4. 关键词关系边表示两篇论文关键词集合在同一篇论文中的共现：边上的 `value` 是共现论文数，即两个关键词共同出现的论文数量。图谱可视化中，连线粗细与透明度按 `sqrt(value / max(value))` 映射到 1—5px 与 16%—58% 透明度（当前数据范围 1—447 篇），悬停节点可查看具体数值。图谱默认只显示共现论文数 ≥ 75（约前 10% 的强关系）的连线，可用"显示全部关系"开关查看全部 1222 条边；过滤仅影响显示、不修改数据。
5. 热度定义为 keyword 覆盖论文数 / 当前筛选范围论文总数 × 1000。
6. 趋势图展示当前数据库记录数，不把演示夹具的数量解释为领域真实发表量。
