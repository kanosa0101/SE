...
3. 关键词关系边表示两篇论文关键词集合在同一篇论文中的共现：边上的 `value` 是共现论文数，即两个关键词共同出现的论文数量。图谱可视化中，连线粗细与透明度按 `sqrt(value / max(value))` 映射到 1—5px 与 16%—58% 透明度（当前数据范围 1—447 篇），悬停节点可查看具体数值。图谱默认只显示共现论文数 ≥ 75（约前 10% 的强关系）的连线，可用"显示全部关系"开关查看全部 1222 条边；过滤仅影响显示、不修改数据。
